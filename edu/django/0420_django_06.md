...
- `api_view` decorator
  - 기본적으로 GET 메소드만 허용되며 다른 메소드 요청에 대해서는 405 Method Not Allowed로 응답
  - View 함수가 응답해야 하는 HTTP 메소드의 목록을 리스트 인자로 받음
...
- N에 해당하는 객체는 생성 시 참조 모델의 객체 정보 필요
  - `.save()` 메소드로 특정 Serializer 인스턴스를 저장하는 과정에서 추가적인 데이터를 받을 수 있음
