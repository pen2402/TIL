...
- 쿠키
  ...
  - 사용자의 행동을 추적하거나 쿠키를 훔쳐서 해당 사용자의 계정 접근 권한을 획득할 수 있음
...
- `logout(request)`
  ...
  - 현재 요청에 대한 session data를 DB에서 완전히 삭제하고, 클라이언트의 쿠키에서도 sessionid가 삭제
...
- is_authenticated
  ...
  - 단 권한과는 관련이 없으며 사용자가 활성화 상태
  - 유효한 세션을 가지고 있는지도 확인하지 않음
...
- login_required decorator
  ...
  - 로그인 이후 "next" 매개변수를 따라 해당 함수로 다시 redirect 되는데, 이때 @require_POST 때문에 405 에러 발생
...
- `UserChangeForm`
  ...
  - 사용 시 문제점
    ...
    - 따라서 UserChangeForm을 상속받아 CustomUserChangeForm
  - CustomUserChangeForm 작성
    - `get_user_model()`
    - User 모델의 fields
  - `get_user_model()`
    - 현재 프로젝트에서 활성화된 사용자 모델을 반환
    - Django는 User 클래스를 직접 참조하는 대신 `get_user_model()` 을 사용하여 참조해야 한다고 강조
  - User 클래스 상속 구조
