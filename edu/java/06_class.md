...
- 매개변수
  ...
  - 메소드가 실행할 때 필요한 데이터를 외부로부터 받는 변수

    - 인자 : 메소드 호출 시 전달되는 실제 값
...
- 매개변수의 개수를 모를 경우
  ...
  - 메소드 호출 시 배열을 넘김으로써 배열의 항목 값들을 모두 전달할 수 있음
    - 배열 항목 수는 호출 시 결정
  ...
  - 메소드 호출 시 넘겨받은 값의 수에 따라 자동으로 배열이 생성되어 해당 값들을 인자로 사용
  - 배열을 직접 인자로 넘길 수도 있음
...
- 리턴값이 있는 메소드
  ...
  - 리턴 타입이 있는 메소드는 반드시 return문을 사용하여 리턴값을 지정해야 함
...
- 리턴값이 없는 메소드
  ...
  - `void`로 선언된 메소드에서도 return문 사용할 수 있음
...
- 객체 내부에서 호출
  ...
  ```java
  <메소드명>([인자, ...]);						// 메소드 호출
  <타입> <변수명> = <메소드명>([인자, ...]);		// 메소드 호출하여 변수에 리턴값 저장
  ```

  - 메소드가 매개변수를 갖고 있는 경우 타입과 수에 맞게 인자 제공하여 호출

- 객체 외부에서 호출

  ```java
  <클래스명> <참조변수명> = new <클래스명>([인자, ...]);
  <참조변수명>.<메소드명>([인자, ...]);						// 리턴값이 없거나 받지 않을 경우
  <타입> <변수명> = <참조변수명>.<메소드명>([인자, ...]);		// 리턴값이 있고 받을 경우
  ```

  - 외부 클래스에서 메소드 호출 시 우선 해당 클래스로부터 객체를 생성해야 함
    - 메소드는 객체에 소속된 멤버로 객체가 전제되어야 함
...
- 인스턴스 멤버
  ...
  - 객체(인스턴스)를 생성한 후 사용할 수 있는 멤버 변수(인스턴스 멤버 변수)와 메소드(인스턴스 메소드)

- `this`

  - 객체 내부에서 인스턴스 멤버에 접근하기 위해 사용되는 키워드

  - 주로 생성자와 메소드의 매개변수 이름이 멤버 변수와 같을 때 인스턴스 멤버임을 명시하고자 사용
...
- 클래스 멤버(정적 멤버)
  ...
  - 클래스에 고정되어 객체를 생성 없이 사용할 수 있는 멤버 변수(클래스 변수)와 메소드(클래스 메소드)
...
- 클래스 멤버 선언
  ...
  public class <클래스명> {
      ...
      static <리턴 타입> <메소드명>([매개변수, ...]) { ... };		// 클래스 메소드
  }
  ```

  - `static` 키워드를 추가로 붙여 선언

- 클래스 로더가 클래스(바이트 코드)를 로드하여 메소드 메모리 영역에 적재할 때 클래스별로 관리됨
...
- 클래스 메소드 선언 시 주의사항
  ...
  - 클래스 메소드 선언 시 내부에 인스턴스 멤버와 `this` 키워드 사용 불가
  ...
  - 클래스 메소드에서 인스턴스 멤버를 사용하려는 경우 객체를 생성한 후 참조 변수로 접근해야 함